...
/*   Updated: 2025/01/29 13:00:08 by gpico-co         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../includes/fdf.h"

// Computes the 3D to 2D projection using scaling, angles, and zoom adjustments.
...
// Populates the 3D grid with point coordinates and assigns default colors.
void	populate_columns(t_map *map, int row)
{
	...
	while (col < map->cols)
	{
		map->max = get_maximum(map->max, convert_to_int(line_parts[col]));
		map->min = get_minimum(map->min, convert_to_int(line_parts[col]));
